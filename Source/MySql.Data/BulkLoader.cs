...
namespace MySql.Data.MySqlClient {
    ...
    public class MySqlBulkLoader {
        ...
        private string BuildSqlCommand() {
            ...
            if ( CharacterSet != null ) sql.AppendFormat( "CHARACTER SET {0} ", CharacterSet );

            var optionSql = new StringBuilder( String.Empty );
            if ( FieldTerminator != DefaultFieldTerminator ) optionSql.AppendFormat( "TERMINATED BY '{0}' ", FieldTerminator );
            if ( FieldQuotationCharacter != Char.MinValue ) optionSql.AppendFormat( "{0} ENCLOSED BY '{1}' ", FieldQuotationOptional ? "OPTIONALLY" : "", FieldQuotationCharacter );
            if ( EscapeCharacter != DefaultEscapeCharacter
                 && EscapeCharacter != Char.MinValue ) optionSql.AppendFormat( "ESCAPED BY '{0}' ", EscapeCharacter );
            ...
            if ( NumberOfLinesToSkip > 0 ) sql.AppendFormat( "IGNORE {0} LINES ", NumberOfLinesToSkip );

            if ( Columns.Count > 0 ) {
                sql.Append( "(" );
                sql.Append( Columns[ 0 ] );
                for ( var i = 1; i < Columns.Count; i++ ) sql.AppendFormat( ",{0}", Columns[ i ] );
                sql.Append( ") " );
            }
            ...
            if ( Expressions.Count > 0 ) {
                ...
                for ( var i = 1; i < Expressions.Count; i++ ) sql.AppendFormat( ",{0}", Expressions[ i ] );
            }
