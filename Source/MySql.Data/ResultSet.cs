...
namespace MySql.Data.MySqlClient {
    internal class ResultSet {
        ...
        public bool Cached {
            ...
            set {
                _cached = value;
                if ( _cached && _cachedValues == null ) _cachedValues = new List<IMySqlValue[]>();
            }
        }
        #endregion
        ...
        public IMySqlValue this[ int index ] {
            get {
                if ( _rowIndex < 0 ) throw new MySqlException( Resources.AttemptToAccessBeforeRead );

                // keep count of how many columns we have left to access
                _uaFieldsUsed[ index ] = true;

                if ( _isSequential && index != _seqIndex ) {
                    if ( index < _seqIndex ) throw new MySqlException( Resources.ReadingPriorColumnUsingSeqAccess );
                    while ( _seqIndex < ( index - 1 ) ) _driver.SkipColumnValue( Values[ ++_seqIndex ] );
        ...
        private bool GetNextRow() {
            var fetched = _driver.FetchDataRow( _statementId, Size );
        ...
        private void CacheClose() { SkippedRows = TotalRows - _rowIndex - 1; }
        ...
        private void LoadColumns( int numCols ) {
            ...
            for ( var i = 0; i < Fields.Length; i++ ) {
                var columnName = Fields[ i ].ColumnName;
                if ( !_fieldHashCs.ContainsKey( columnName ) ) _fieldHashCs.Add( columnName, i );
                if ( !_fieldHashCi.ContainsKey( columnName ) ) _fieldHashCi.Add( columnName, i );
                Values[ i ] = Fields[ i ].GetValueObject();
            }
        }

        private void ReadColumnData( bool outputParms ) {
            for ( var i = 0; i < Size; i++ ) Values[ i ] = _driver.ReadColumnValue( i, Fields[ i ], Values[ i ] );

            // if we are caching then we need to save a copy of this row of data values
            if ( Cached ) _cachedValues.Add( (IMySqlValue[]) Values.Clone() );
