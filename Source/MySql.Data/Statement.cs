...
namespace MySql.Data.MySqlClient {
    internal abstract class Statement {
        ...
        protected virtual void BindParameters() {
            ...
            while ( true ) {
                ...
                while ( index < Command.Batch.Count ) {
                    var batchedCmd = Command.Batch[ index++ ];
                    var packet = _buffers[ _buffers.Count - 1 ];

                    // now we make a guess if this statement will fit in our current stream
                    var estimatedCmdSize = batchedCmd.EstimatedSize();
            ...
            while ( token != null ) {
                ...
                if ( MySqlTokenizer.IsParameter( token ) ) {
                    if ( ( !parameters.ContainsUnnamedParameters && token.Length == 1 && parameterCount > 0 )
                         || parameters.ContainsUnnamedParameters && token.Length > 1 ) throw new MySqlException( Resources.MixedParameterNamingNotAllowed );

                    parameters.ContainsUnnamedParameters = token.Length == 1;
                    if ( SerializeParameter( parameters, packet, token, parameterCount ) ) token = null;
                    parameterCount++;
                }
                if ( token != null ) {
                    if ( sqlServerMode
        ...
        private bool SerializeParameter( MySqlParameterCollection parameters, MySqlPacket packet, string parmName, int parameterIndex ) {
            ...
            else throw new MySqlException( Resources.ParameterIndexNotFound );
