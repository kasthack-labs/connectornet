    ...
    internal class MyNetworkStream : NetworkStream {
        ...
        private void HandleOrRethrowException( Exception e ) {
            ...
            while ( currentException != null ) {
                ...
                if ( exception != null ) {
                    ...
                    if ( IsTimeoutException( socketException ) ) return;
        ...
        public override int Read( byte[] buffer, int offset, int count ) {
            ...
            if ( exception.GetBaseException() is SocketException
                 && IsTimeoutException( (SocketException) exception.GetBaseException() ) ) throw new TimeoutException( exception.Message, exception );
            throw exception;
        }
        ...
        private static IPHostEntry GetHostEntry( string hostname ) {
            var ipHe = ParseIpAddress( hostname );
        ...
        private static MyNetworkStream CreateSocketStream( MySqlConnectionStringBuilder settings, IPAddress ip, bool unix ) {
            ...
            if ( settings.Keepalive > 0 ) SetKeepAlive( socket, settings.Keepalive );
            ...
            var stream = new MyNetworkStream( socket, true );
        ...
        private static void SetKeepAlive( Socket s, uint time ) {
            ...
            else timeMilliseconds = time * 1000;
