...
namespace MySql.Data.MySqlClient {
    ...
    public sealed partial class MySqlParameterCollection {
        ...
        public override void Clear() {
            foreach ( var p in _items ) p.Collection = null;
            _items.Clear();
            _indexHashCs.Clear();
            _indexHashCi.Clear();
        }
        ...
        private void CheckIndex( int index ) {
            if ( index < 0
                 || index >= Count ) throw new IndexOutOfRangeException( "Parameter index is out of range." );
        }
        ...
        private void InternalSetParameter( string parameterName, MySqlParameter value ) {
            var index = IndexOf( parameterName );
            if ( index < 0 ) throw new ArgumentException( "Parameter '" + parameterName + "' not found in the collection." );
            InternalSetParameter( index, value );
        }

        private void InternalSetParameter( int index, MySqlParameter value ) {
            var newParameter = value;
            if ( newParameter == null ) throw new ArgumentException( Resources.NewValueShouldBeMySqlParameter );
        ...
        public override int IndexOf( string parameterName ) {
            ...
            if ( !_indexHashCs.TryGetValue( parameterName, out i )
                 && !_indexHashCi.TryGetValue( parameterName, out i ) ) return -1;
            return i;
        }
        ...
        public override int IndexOf( object value ) {
            var parameter = value as MySqlParameter;
            if ( null == parameter ) throw new ArgumentException( "Argument must be of type DbParameter", "value" );
        ...
        private MySqlParameter InternalAdd( MySqlParameter value, int index ) {
            ...
            if ( String.IsNullOrEmpty( value.ParameterName ) ) value.ParameterName = String.Format( "Parameter{0}", GetNextIndex() );

            // make sure we don't already have a parameter with this name
            if ( IndexOf( value.ParameterName ) >= 0 ) throw new MySqlException( String.Format( Resources.ParameterAlreadyDefined, value.ParameterName ) );
            var inComingName = value.ParameterName;
            if ( inComingName[ 0 ] == '@'
                 || inComingName[ 0 ] == '?' ) inComingName = inComingName.Substring( 1, inComingName.Length - 1 );
            if ( IndexOf( inComingName ) >= 0 ) throw new MySqlException( String.Format( Resources.ParameterAlreadyDefined, value.ParameterName ) );
        ...
        private void AdjustHashes( int keyIndex, bool addEntry ) {
            for ( var i = 0; i < Count; i++ ) {
                var name = _items[ i ].ParameterName;
