...
namespace MySql.Data.MySqlClient {
    ...
    internal sealed class MySqlPool {
        ...
        public MySqlPool( MySqlConnectionStringBuilder settings ) {
            ...
            for ( var i = 0; i < _minSize; i++ ) EnqueueIdle( CreateNewPooledConnection() );
        ...
        private Driver GetPooledConnection() {
            ...
            lock ( ( _idlePool as ICollection ).SyncRoot ) {
                if ( HasIdleConnections ) driver = _idlePool.Dequeue();
            }
            ...
            if ( driver != null )
                ...
                if ( !driver.Ping() ) {
                    ...
                    // do so now
                    driver.Reset();
            if ( driver == null ) driver = CreateNewPooledConnection();
        ...
        public void ReleaseConnection( Driver driver ) {
            lock ( ( _inUsePool as ICollection ).SyncRoot ) {
                if ( _inUsePool.Contains( driver ) ) _inUsePool.Remove( driver );
            }
        ...
        public void RemoveConnection( Driver driver ) {
            ...
            if ( BeingCleared && NumConnections == 0 ) MySqlPoolManager.RemoveClearedPool( this );
        }

        private Driver TryToGetDriver() {
            var count = Interlocked.Decrement( ref _available );
            if ( count < 0 ) {
                Interlocked.Increment( ref _available );
                return null;
            }
        ...
        internal List<Driver> RemoveOldIdleConnections() {
            ...
            lock ( ( _idlePool as ICollection ).SyncRoot ) {
                ...
                while ( _idlePool.Count > _minSize ) {
                    var d = _idlePool.Peek();
                    var expirationTime = d.IdleSince.Add( new TimeSpan( 0, 0, MySqlPoolManager.MaxConnectionIdleTime ) );
